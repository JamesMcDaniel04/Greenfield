...
export default function MyIdeaDetailPage() {
  ...
  const ideaQuery = useUserIdea(id);
  const [role, setRole] = useState<AgentRole>("research");
  const [prompt, setPrompt] = useState("");

  const subject = id ? { user_idea_id: id } : null;
  const runsQuery = useByoAgentRuns(subject, role);
  const runAgent = useRunByoAgent(subject);

  if (!isSupabaseConfigured) return <Navigate to="/my-ideas" replace />;
  if (loading) return null;
  if (!user) return <Navigate to={`/auth?mode=signin&next=/my-ideas/${id}`} replace />;
  if (!byo.unlocked) return <Navigate to="/my-ideas" replace />;
  if (ideaQuery.isLoading) return <p className="container-wide py-10 text-muted-foreground">Loading…</p>;
  if (!ideaQuery.data) return <Navigate to="/my-ideas" replace />;
  ...
  return (
    <section className="container-wide py-10 max-w-5xl">
      <Link to="/my-ideas" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" />
        Back to ideas
      </Link>

      <header className="mt-4">
        <h1 className="font-display text-3xl">{idea.title}</h1>
        <p className="mt-2 text-base text-muted-foreground">{idea.one_liner}</p>
      </header>
